...
def search_computers(conn):
    ...
    try:
        ...
        driver.find_element_by_css_selector('.col-lg-3:nth-child(1) .mask').click()

        elem = driver.find_element_by_xpath("//div[2]/div/form/button")
        hover = ActionChains(driver).move_to_element(elem)
        hover.perform()
        elem.click()

        driver.find_element_by_xpath('(//a[contains(@href, "/")])[2]').click()
    except Exception as e:
